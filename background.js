...
async function handleLookupWord(rawWord, targetLang) {
  ...
  // Target-language "one-line gloss": translate the word into the target language via DeepL.
  // glossStatus tells the popup WHY there's no gloss (no key / error) so it can show a hint,
  // instead of silently showing only the English definitions (which looks broken).
